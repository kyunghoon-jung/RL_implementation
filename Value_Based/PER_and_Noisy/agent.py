...
class Agent:
    ...
    def select_action(self, state: 'Must be pre-processed in the same way while updating current Q network. See def _compute_loss'):
        ...
        if np.random.random() < self.epsilon:
            ...
            Qs = self.q_current(torch.FloatTensor(state).to(self.device).unsqueeze(0)/255)
    ...
    def train(self):
        ...
        for frame_idx in range(1, self.num_frames+1):
            ...
            reward, next_state, done = self.get_state(state, action, skipped_frame=self.skipped_frame)
            print(f"(R:{reward}, A:{action}), Qs: {np.round(Qs, 3)}", end='\r')
            self.store(state, action, np.clip(reward, -1, 1), next_state, done) # Store a reward clipped btw -1 ~ 1
            history_store.append([state, Qs, action, reward, next_state, done])

            current_update_count += 1
            if (current_update_count % self.current_update_freq) == 0:
            ...
            if done:
                if score > max(scores):
                    torch.save(self.q_current.state_dict(), self.model_path+'{}_Highest_Score_{}.pt'.format(frame_idx, score))
                ...
                if np.mean(scores[-10:]) > max(avg_scores):
                    torch.save(self.q_current.state_dict(), self.model_path+'{}_Avg_Score_{}.pt'.format(frame_idx, np.mean(scores[-10:])))
                    training_time = round((time.time()-tic)/3600, 1)
