...
class Agent:
    def __init__(self, 
                 ...
                 initial_std: ('float: the standard deviation of noise'),
        ...
        self.n_atoms = n_atoms                            # Variables for C51
        self.Vmin = Vmin                                  # Variables for C51
        self.Vmax = Vmax                                  # Variables for C51
        self.dz = (Vmax - Vmin) / (n_atoms - 1)           # Variables for C51
        self.support = torch.linspace(Vmin, Vmax, n_atoms).to(self.device) # Variables for C51
    ...
    def select_action(self, state: 'Must be pre-processed in the same way while updating current Q network. See def _compute_loss'):
        ...
        return Qs.detach().cpu().numpy()[0], Expected_Qs.detach().cpu().numpy()[0], action.item()
    ...
    def train(self):
        ...
        for frame_idx in range(1, self.num_frames+1):
            ...
            if done:
                ...
                self.plot_status(self.plot_option, frame_idx, score, self.scores, epi_Qs, losses)
                state = self.get_init_state()
                history_store = []
                score = 0
                epi_Qs = []
    ...
    def test(self):
        ...
        episode_history = []
        state = self.get_init_state()
        print("C51 Supports: ", self.support)
        for frame_idx in range(1000000):
            ...
            print('Frame: ', frame_idx, "Expexted_Q_values: ", np.round(Exp_Qs, 3), "Action: ", action, "Reward: ", reward, "Accumulated Reward:", rewards)
            if done: break
    ...
    def _render(self, state, action, Qs):
        if self.is_render:
            ...
            for i in range(self.input_frames):
                self.vis.image(
                    state[0], 
                    opts={'title': f'State num: {i}. Action: {action}'},
    ...
    def plot_status(self, is_plot, frame_idx, score, scores, epi_Qs, losses):
        if is_plot=='inline': 
